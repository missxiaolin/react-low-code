...
export default () => {
  const variableRef = useRef<{
    open: (type: "add" | "edit", variable?: PageVariable) => void;
  }>();
  // 页面组件
  const { page, removeVariable } = usePageStore((state) => state);
      ...
      <List
        ...
        renderItem={(item) => {
          return (
            <List.Item
              actions={[
                <a onClick={(event) => handleEdit(event, item)}>修改</a>,
                <a onClick={(event) => handleRemove(event, item.name)}>删除</a>,
              ]}
            >
              <List.Item.Meta
                title={`${item.name}`}
