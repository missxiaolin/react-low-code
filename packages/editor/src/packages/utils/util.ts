...
export const formatDate = (
  date?: Date | string,
  rule: string = "YYYY-MM-DD HH:mm:ss"
) => {
...
export const getDateRangeByType = (type: string) => {
  ...
  // 今天、本周、本月、本年
  if (
    ["today", "curWeek", "curMonth", "curYear", "curQuarter"].includes(type)
  ) {
    if (type == "today") {
      //默认即为当天
  ...
  // 昨天、上周、上月
  if (
    ["yesterday", "lastWeek", "lastMonth", "last3Month", "lastYear"].includes(
      type
    )
  ) {
    if (type == "yesterday") {
...
export function renderFormula(formula: string, eventParams: any = {}) {
  try {
    ...
    const formIds: Array<string> =
      formula.match(/([A-Za-z]+_\w+)\.[\w\.]*/g) || [];
    const originIds: Array<string> = [
      ...new Set(formIds.map((id) => id.split(".")[0])),
    ];
    const fnParams: Array<string> = ["context", "eventParams"];
    const { page: pageStore } = usePageStore.getState();
  ...
  dates.map((id: string) => {
    ...
    if (["DatePicker", "TimePicker"].includes(type)) {
      ...
      if (values[name]?.length == 2) {
        const [start, end] = values[name]?.map(
          (date: any) => date?.format?.(format) || dayjs(values[date])
        ) || [undefined, undefined];
...
export const getEnv = () => {
  ...
  const env = (result1 ? result1[1] : result2 ? result2[1] : "stg") as
    | "stg"
    | "pre"
    | "prd";
  return {
