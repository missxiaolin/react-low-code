...
const Toolbar = memo(
  ({ hoverTarget, copyElement, pastElement, delElement }: any) => {
    let nodeNav: any = {};
    // 页面组件
    const {
      selectedElement,
    ...
    const init = () => {
      const currentId = selectedElement?.id; // 当前选中的组件id
      const element = currentId ? page.elementsMap[currentId] : null;
      if (element) {
        // 查找父节点
        if (element.parentId) {
          const { id, type, name } = page.elementsMap[element.parentId];
        ...
        // 查找子节点
        const childElement = Object.values(page.elementsMap).find(
          (item: any) => item.parentId === element.id
        );
        if (childElement) {
          nodeNav["child"] = {
            id: childElement.id,
    ...
    return (
      <>
        <div
          className={selectedElement ? "toolbar-box selected" : "toolbar-box"}
          style={selectedStyle}
          id="editorToolbar"
        >
          <div className={"tool-bar " + direction}>
            ...
            <div className="node-nav">
              {nodeNav.parent && (
                <span
                  className="node-tip parent"
                  onClick={(event) => handleNav(event, nodeNav.parent)}
                >
                  {nodeNav.parent?.name || nodeNav.parent?.type}
                </span>
              )}
              <span
                className="node-tip current"
                onClick={(event) => handleNav(event, nodeNav.current)}
              >
                {nodeNav.current?.name || nodeNav.current?.type}
              </span>
              {nodeNav.child && (
                <span
                  className="node-tip child"
                  onClick={(event) => handleNav(event, nodeNav.child)}
                >
            ...
            <Space className="actions">
              ...
              <Tooltip title="下移动">
                <ArrowDownOutlined
                  onClick={(event) => moveCard(event, "down")}
                />
              </Tooltip>
              <Tooltip title="复制">
                <CopyOutlined
        ...
        {/* 组件虚浮样式 */}
        <div
          className={hoverTarget ? "toolbar-box hover" : "toolbar-box"}
          style={hoverStyle}
        ></div>
      </>
    );
