...
const EventConfig = memo(() => {
  ...
  const nodeRef = useRef<{
    open: (nodeList: NodeType[], callback: callback) => void;
  }>();
  ...
  // 定义事件列表
  const items: Array<{ key: string; label: string }> | undefined = state
    .selectedElement?.id
  ...
  const handleAddAction = (index: number) => {
    nodeRef.current?.open(
      form.getFieldValue(["events", index, "actions"]),
      (nodeList: any) => {
                ...
                {fields.map(({ key, name }) => (
                  <div key={"event" + key} className={styles.eventFlow}>
                    ...
                    <h2 className={styles.title}>
                      <span>
                        {form.getFieldValue(["events", name, "nickName"])}
                      </span>
                      {/* 删除事件 */}
                      <DeleteOutlined
                        onClick={() => remove(name)}
