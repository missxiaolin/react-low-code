...
const StyleConfig: any = () => {
  ...
  useEffect(() => {
    ...
    if (!state.selectedElement) {
      form.setFieldValue(
        "scopeCss",
        state.page.config.scopeCss || "/* 请在此处添加样式*/\n.marsview{\n\n}"
      );
      form.setFieldValue("scopeStyle", state.page.config.scopeStyle);
    } else {
      // 填充组件样式
      const config =
        state.page.elementsMap[state.selectedElement.id]?.config || {};
      form.setFieldValue(
        "scopeCss",
        config.scopeCss || "/* 请在此处添加样式*/\n.marsview{\n\n}"
      );
  ...
  return (
    ...
    >
      ...
      <Form.Item
        name={["scopeStyle", "backgroundImage"]}
        label={"图片"}
        tooltip="支持渐变色。图片使用时，直接输入远程地址：http(s)://xxx.png"
      >
        <BackgroundImage />
      </Form.Item>
      <Form.Item
        name={["scopeStyle", "backgroundSize"]}
        label={"尺寸"}
        tooltip="默认的时候，可以手动输入尺寸"
      >
        <BackgroundSize />
      </Form.Item>
      ...
      <Form.Item
        key={"zIndex"}
        name={["scopeStyle", "zIndex"]}
        label={"zIndex"}
      >
        <InputNumber placeholder="层级" />
      </Form.Item>
      {!["", undefined, "static"].includes(
        form.getFieldValue(["scopeStyle", "position"])
      ) && (
        <Form.Item label="位置">
          <Flex gap={3}>
