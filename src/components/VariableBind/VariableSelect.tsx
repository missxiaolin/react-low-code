...
const SelectVariableModal: any = (
  ...
  const [visible, setVisible] = useState(false); // 变量选择弹窗
  const [form] = Form.useForm();

  const { page } = usePageStore((state) => state);
  const { variables, pageName, elements, elementsMap } = page;
  ...
  const getFormAndTable = useCallback(() => {
    ...
    Object.keys(elementsMap).map((id) => {
      if (
        id.startsWith("SearchForm_") ||
        id.startsWith("Form_") ||
        id.startsWith("GridForm_") ||
        id.startsWith("MarsTable_")
      ) {
  ...
  const handleSelect = (selectedKeys: any, { node }: any) => {
    ...
    if (node.type === "Store") {
      form.setFieldValue(
        "expression",
        `${beforeExpression} context.store.${node.id}`.trimStart()
      );
      return;
    }
    // 选择页面全局变量
    if (node.type === "Variable") {
      form.setFieldValue(
        "expression",
        `${beforeExpression} context.variable.${node.id}`.trimStart()
      );
      return;
    }
    ...
    if (node.type === "PageVariable") {
      ...
      if (name)
        form.setFieldValue(
          "expression",
          `${beforeExpression} context.${node.parentId}.${name}`.trimStart()
        );
    } else if (node.type === "Form" || node.type === "SearchForm") {
      form.setFieldValue(
        "expression",
        `${beforeExpression}  context.${node.id}`
      );
    } else {
      const formItem = elementsMap[node.id]?.config.props.formItem;
      form.setFieldValue(
        "expression",
        `${beforeExpression} context.${node.parentId}.${formItem.name}`.trimStart()
      );
    }
  };
  ...
  const handleClick = (type: string) => {
    ...
    form.setFieldValue(
      "expression",
      `${beforeExpression} context.${type}()`.trimStart()
    );
  };
            ...
            <Popover
              content={
                <div>
                  <p>
                    1.
                    默认支持普通变量定义，最终会返回该解析后的变量值：variable.current{" "}
                  </p>
                  <p>2. 支持三元表达式：userName === 'jack' ? 1 : 2 </p>
                  <p>2. 支持布尔值：true ，最终会被解析会true变量进行返回。</p>
                  <p>3. 支持逻辑判断：if( a {">"} 1 ) return 1; return 2 </p>
                  <p>
                    4. 支持函数function定义：function getName()
                    {'{ return "jack" } '}
                  </p>
                  <p>
                    5.
                    定义函数的时候，不支持自定义参数，参数只能是从右侧选择的变量或者表单对象，系统会自动解析。
                  </p>
                  <p>
                    6. 表格自定义渲染时，需要手工定义渲染函数：function
