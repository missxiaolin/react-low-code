    ...
    <>
      {attrs.map((item: SchemaType, index) => {
        ...
        const key =
          item.key ||
          item.name?.toString() ||
          item.label?.toString() + index.toString();
        let FormControl = <></>;
        if (item.type == "Title") {
          if (item.popover) {
            return (
              <Popover
                ...
                placement={item.popover.placement || "left"}
                key={key}
              >
          ...
          FormControl = (
            <Select {...item.props} suffixIcon={<CaretDownOutlined />} />
          );
        } else if (item.type == "Radio") {
          FormControl = (
            <Radio.Group {...item.props} suffixIcon={<CaretDownOutlined />} />
          );
        } else if (item.type == "InputNumber") {
          FormControl = (
            <InputNumber {...item.props} style={{ width: "100%" }} />
          );
